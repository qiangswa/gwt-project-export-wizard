...
public class ArchiveUtil {
	...
	public static java.io.File createTempFile(String baseName)
			throws IOException {
	...
	public static java.io.File createTempFile(String baseName,
		...
		java.io.File tempFile = java.io.File.createTempFile(fileName, null,
				directory);
	 ...
	 */
	public static List<File> getWriteProtectedFiles(java.io.File aFile,
			List<File> aList) {
		if (aList == null)
	...
	public static void checkWriteable(java.io.File aFile)
	...
	public static boolean isRenameable(java.io.File orig) {
		...
		try {
			if (orig.getParent() != null)
				baseName = new java.io.File(orig.getParent(), baseName)
						.getCanonicalPath();
		} catch (java.io.IOException ex) {
			return false;
		}
	...
	public static void cleanupAfterTempSave(String aUri, java.io.File original,
			java.io.File destinationFile) throws ArchiveSaveFailureException {
	...
	/**
	 * Utility to create ZipFiles which avoid memory leaks because closing them
	 * fails to close open inputstreams. There is a SUN bug open for this:
	 * http://bugs.sun.com/bugdatabase/view_bug.do?bug_id=6735255 but it looks
	 * like the "fix" will be to change the Javadoc :-(
	 * 
	 * @param aFile
	 *            mode
	 * @return
	 ...
	 * @throws IOException
	 */
	public static ZipFile newZipFile(File aFile, int mode) throws ZipException,
			IOException {
		return new ZipFile(aFile, mode) {
			...
			private void closeOpenStreams() {
				if (openStreams != null) {
					for (Iterator<InputStream> iterator = openStreams
